...
// If dist/sitemap.xml already exists, leave it alone.
if (fs.existsSync(sitemapXml)) {
  console.log('sitemap.xml already present in dist/ — no action needed.');
  process.exit(0);
}

// Prefer sitemap-0.xml (plugin sometimes emits sitemap-0.xml)
if (fs.existsSync(sitemap0)) {
  copyIfNeeded(sitemap0, sitemapXml);
  process.exit(0);
}

// Fallback: if only sitemap-index.xml exists, copy it to sitemap.xml
if (fs.existsSync(sitemapIndex)) {
  copyIfNeeded(sitemapIndex, sitemapXml);
  process.exit(0);
}
